...
class attachment_act extends act {
    ...
    function attachment_js_action() {
        ...
        if(!rank::arcget($aid,$this->view->usergroupid,'down')) {
            $link="<a id='att' href='javascript:alert(\"未取得授权,无法下载!\");'><img src='images/download.gif' alt='$name' /></a>";
            echo tool::text_javascript($link);
            exit;
        }else {
            if(config::get('verifycode')) {
                $link="<a target='_blank' id='att' href='".url::create('attachment/downfile/aid/'.$aid.'/v/ce')."'><img src='images/download.gif' alt='$name' /></a>";
                echo tool::text_javascript($link);
                exit;
            }else {
                $link="<a target='_blank' id='att' href='".url::create('attachment/down/aid/'.$aid)."'><img src='images/download.gif' alt='$name' /></a>";
                echo tool::text_javascript($link);
                exit;
            }
        }
    ...
    function downfile_action() {
        if(front::post('submit')) {
            if(!session::get('verify') ||front::post('verify')<>session::get('verify')) {
                ...
                if(!rank::arcget($aid,$this->view->usergroupid,'down'))
                    $link="<p align='center'><a id='att' href='javascript:alert(\"未取得授权,无法下载!\");'><img src='images/download.gif' alt='$name' border='0' /></a></p>";
                else $link="<p align='center'><a id='att' href='".url::create('attachment/down/aid/'.$aid)."'><img src='images/download.gif' alt='$name' border='0' /></a></p>";
                echo $link;
                exit;
            }
        }
