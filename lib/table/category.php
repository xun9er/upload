...
class category extends table {
    ...
    function getcols($act) {
        if ($act == 'modify')
            return 'catid,catname,parentid,htmldir,template,listtemplate,showtemplate,ishtml,isshow,ispages,includecategories,linkto,htmlrule,listhtmlrule,showhtmlrule,mtitle,description,keyword,scategory,isnav,islast,attr3';
    ...
    function init() {
        $_category=$this->getrows(null,1000,'`listorder` desc,1');
        $category=array();
    ...
    static function url($catid,$page=null,$relative=false) {
        if(front::get('t')=='wap')  return url::create('archive/list/t/wap/catid/'.$catid.'/page/'.$page);
        $category=self::getInstance();
