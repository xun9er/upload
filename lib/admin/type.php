...
class type extends table {
    ...
    function getcols($act) {
        if ($act == 'modify')
            return 'typeid,typename,parentid,htmldir,template,listtemplate,showtemplate,ishtml,isshow,ispages,includecatarchives,linkto,htmlrule,listhtmlrule,showhtmlrule,mtitle,description,keyword,stype,islast,thumb,thumb_width,thumb_height';
        else
            return 'typeid,typename,htmldir,isshow,ispages,linkto,stype';
    }
    function get_form() {
        return array(
                'parentid'=>array(
    ...
    static function image($typeid) {
        $type=self::getInstance();
        if (isset($type->type[$typeid]['image']))
            return config::get('base_url').'/'.$type->type[$typeid]['image'];
        else
            return '';
    }
    ...
    static function gettemplate($typeid,$tag='listtemplate',$up=true) {
        if (!$typeid &&front::get('parentid'))
            $typeid=front::get('parentid');
        $type=self::getInstance();
        if (@$type->type[$typeid]['template'] &&$tag == 'listtemplate')
            return $type->type[$typeid]['template'];
        if (@$type->type[$typeid][$tag])
            return $type->type[$typeid][$tag];
        if (!$up)
            return;
        ...
        foreach ($parents as $pid) {
            if ($pid == $typeid)
                continue;
            if (@$type->type[$pid][$tag])
                return $type->type[$pid][$tag];
        }
        $default=array(
                'listtemplate'=>'type/list.html',
        );
        if (isset($default[$tag]))
            return $default[$tag];
    }
    static function gethtmlrule($typeid,$tag='listhtmlrule') {
        if (!$typeid &&front::get('parentid'))
            $typeid=front::get('parentid');
        $type=self::getInstance();
        if (@$type->type[$typeid]['htmlrule'] &&$tag == 'listhtmlrule')
            return $type->type[$typeid]['htmlrule'];
        $parents=$type->getparents($typeid,true);
        ksort($parents);
        foreach ($parents as $pid) {
            if ($pid == $typeid)
    ...
    static function getishtml($typeid) {
        if (config::get('list_page_php') == '1')
            return true;
        if (config::get('list_page_php') == '2')
            return false;
        $type=self::getInstance();
        if (@$type->type[$typeid]['ishtml'] == '1')
            return true;
        $parents=$type->getparents($typeid,true);
        ksort($parents);
        foreach ($parents as $pid) {
            if ($pid == $typeid)
                continue;
            if (@$type->type[$pid]['ishtml'] == '1')
                return true;
            if (@$type->type[$pid]['ishtml'] == '2')
                return false;
        }
        return false;
    }
    static function getarcishtml($arc) {
        if (config::get('show_page_php') == '1')
            return true;
        if (config::get('show_page_php') == '2')
            return false;
        if ($arc['ishtml'])
            return true;
        if (self::getishtml($arc['typeid']))
            return true;
        return false;
    }
    static function getattr($typeid,$attr) {
        $type=self::getInstance();
        if (@$type->type[$typeid][$attr])
            return $type->type[$typeid][$attr];
        $parents=$type->getparents($typeid,true);
        ksort($parents);
        foreach ($parents as $pid) {
            if ($pid == $typeid)
                continue;
            if (@$type->type[$pid][$attr])
                return $type->type[$typeid][$attr];
        }
        return false;
    }
    ...
    static function check($typeid,$tag='isnotlast') {
        ...
        if ($tag == 'islast'&&!$type['islast'])
            return false;
        if ($tag == 'isnotlast'&&$type['islast'])
            return false;
        if ($tag == 'tolast') {
            if ($_type->type[$typeid]['islast'])
                return true;
            $sons=$_type->sons($typeid);
            foreach ($sons as $tid) {
                if ($_type->type[$tid]['islast'])
                    return true;
            }
            return false;
        }
