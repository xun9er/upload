...
if($do == 'add' and $username != '')
{
	...
	@$result = $db->my_fetch_array($sql);
	if(count($result) == 0) {
	$sql = "INSERT INTO `operators` (`username`,`password`,`firstname`,`level`,`timestamp`) VALUES('".$username."','".$password."','".$realname."','".$level."','".$timestamp."')";
	$db->query($sql);
	}
}
...
for($i=0;$i<count($result);$i++)
{
	...
	$list = $list."
	<tr class=\"light\">
      <td align=\"right\">".$realname."(".$username.")<br />[".$level."]-[".$login."]</td>
